...
class InventoryPhaseTwoTest extends TestCase
{
    ...
    public function test_only_one_default_warehouse_is_kept_per_branch(): void
    {
        Sanctum::actingAs($this->admin()); $branch=Branch::where('code','PRINCIPAL')->firstOrFail();
    ...
    {
        $admin=$this->admin(); Sanctum::actingAs($admin); $warehouse=$this->warehouse(); $product=$this->product();
    ...
    public function test_sku_is_generated_normalized_unique_and_stable(): void
    {
        ...
        $sku=$manual->sku; $manual->update(['name'=>'Otro nombre']); $this->assertSame($sku,$manual->refresh()->sku);
    ...
    {
        $admin=$this->admin(); $product=$this->product(); $warehouse=$this->warehouse(); $service=app(InventoryService::class);
    ...
    {
        $product=$this->product(); $warehouse=$this->warehouse(); $service=app(InventoryService::class);
        $service->manualIn($product,$warehouse,2,'Ingreso');
        foreach ([0,3] as $quantity) {
            try {$service->manualOut($product,$warehouse,$quantity,'Salida');$this->fail('La salida debió ser rechazada.');} catch (\App\Exceptions\InventoryException) {}
        }
    ...
    {
        $product=$this->product(); $source=$this->warehouse(); $destination=Warehouse::create(['branch_id'=>$source->branch_id,'code'=>'DEST','name'=>'Destino','is_default'=>false,'is_active'=>true]); $service=app(InventoryService::class);
    ...
    {
        $product=$this->product();$source=$this->warehouse();$destination=Warehouse::create(['branch_id'=>$source->branch_id,'code'=>'DEST-FAIL','name'=>'Destino','is_default'=>false,'is_active'=>true]);$service=app(InventoryService::class);$service->manualIn($product,$source,2,'Ingreso');
        foreach ([fn()=>$service->transfer($product,$source,$source,1,'Inválido'),fn()=>$service->transfer($product,$source,$destination,3,'Exceso')] as $operation){try{$operation();$this->fail('El traslado debió fallar.');}catch(\App\Exceptions\InventoryException){}}
        $this->assertSame(2,$product->refresh()->stock);$this->assertFalse(WarehouseInventory::where(['product_id'=>$product->id,'warehouse_id'=>$destination->id])->exists());
    }
    ...
    public function test_movements_cannot_be_edited_or_deleted(): void
    {
        ...
        try{$movement->update(['reason'=>'Alterado']);$this->fail('No se debe editar.');}catch(LogicException){}
    ...
    {
        $admin=$this->admin();$product=$this->product(['sku'=>'FILTRO-01']);app(InventoryService::class)->manualIn($product,$this->warehouse(),4,'Ingreso',$admin);
        Sanctum::actingAs($admin);$this->getJson('/api/v1/admin/inventories?search=FILTRO-01')->assertOk()->assertJsonCount(1,'data');
        $this->getJson('/api/v1/admin/inventory-movements')->assertOk()->assertJsonStructure(['data','current_page','last_page']);
        Sanctum::actingAs(User::factory()->create(['role'=>'customer']));$this->getJson('/api/v1/admin/inventories')->assertForbidden();
    }

    public function test_public_stock_uses_only_checkout_warehouse_while_admin_stock_is_global(): void
    {
        $product=$this->product();$principal=$this->warehouse();
        $secondary=Warehouse::create(['branch_id'=>$principal->branch_id,'code'=>'SECONDARY','name'=>'Secundario','is_default'=>false,'is_active'=>true]);
        $service=app(InventoryService::class);$service->manualIn($product,$principal,2,'Ingreso vendible');$service->manualIn($product,$secondary,10,'Stock interno');
    ...
    {
        $customer=User::factory()->create(['role'=>'customer']);$product=$this->product(['stock'=>5]);Sanctum::actingAs($customer);
        $orderId=$this->postJson('/api/v1/orders',$this->orderPayload($product,2))->assertCreated()->json('id');
        $item=OrderItem::with(['product','warehouse'])->where('order_id',$orderId)->firstOrFail();$service=app(InventoryService::class);
        $service->sell($item,$customer);$this->assertSame(3,$product->refresh()->stock);
        $this->assertSame(1,InventoryMovement::where('idempotency_key','sale-order-item-'.$item->id)->count());
        $service->returnCancellation($item,$customer);$service->returnCancellation($item,$customer);
        $this->assertSame(5,$product->refresh()->stock);
    ...
    {
        $product=$this->product(['stock'=>1]);$movement=InventoryMovement::where('idempotency_key','initial-product-'.$product->id)->firstOrFail();
        $this->expectException(\Illuminate\Database\QueryException::class);
    ...
    {
        $product=$this->product(['stock'=>5]);$warehouse=$this->warehouse();$service=app(InventoryService::class);
        $increase=$service->adjust($product,$warehouse,8,'Conteo alto');
        $this->assertSame(8,$increase->quantity_after);$this->assertSame(8,$increase->metadata['counted_quantity']);$this->assertSame(3,$increase->metadata['difference']);
        $reduction=$service->adjust($product,$warehouse,2,'Conteo bajo');
        $this->assertSame(2,$reduction->quantity_after);$this->assertSame(-6,$reduction->metadata['difference']);
        $count=InventoryMovement::where('type',InventoryMovement::CORRECTION)->count();
        try{$service->adjust($product,$warehouse,2,'Sin diferencia');$this->fail('Un saldo sin cambios debe rechazarse.');}catch(\App\Exceptions\InventoryException){}
    ...
    {
        $product=$this->product(['stock'=>3]);$source=$this->warehouse();$destination=Warehouse::create(['branch_id'=>$source->branch_id,'code'=>'ROLLBACK-DEST','name'=>'Destino rollback','is_default'=>false,'is_active'=>true]);
        $before=InventoryMovement::count();
        try{app(InventoryService::class)->transfer($product,$source,$destination,4,'Debe fallar');$this->fail('El traslado debía fallar.');}catch(\App\Exceptions\InventoryException){}
    ...
    {
        $customer=User::factory()->create(['role'=>'customer']);$first=$this->product(['stock'=>2]);$second=$this->product(['stock'=>1]);Sanctum::actingAs($customer);
        $payload=$this->orderPayload($first,1);$payload['items'][]=['product_id'=>$second->id,'quantity'=>2];
        $this->postJson('/api/v1/orders',$payload)->assertUnprocessable();
        $this->assertDatabaseCount('orders',0);$this->assertDatabaseCount('order_items',0);
        $this->assertSame(2,$first->refresh()->stock);$this->assertSame(1,$second->refresh()->stock);
    ...
    public function test_inventory_filters_cover_branch_warehouse_name_and_sku(): void
    {
        ...
        $warehouse=Warehouse::create(['branch_id'=>$branch->id,'code'=>'FILTER-WH','name'=>'Almacén filtro','is_default'=>true,'is_active'=>true]);
        $product=$this->product(['name'=>'Lubricante Buscable','sku'=>'FILTER-SKU']);app(InventoryService::class)->manualIn($product,$warehouse,7,'Ingreso filtro');Sanctum::actingAs($this->admin());
    ...
    public function test_movements_have_no_update_or_delete_routes_and_stock_cannot_be_overwritten(): void
    {
        foreach(Route::getRoutes() as $route){
            if(str_contains($route->uri(),'inventory-movements'))$this->assertEmpty(array_intersect($route->methods(),['PUT','PATCH','DELETE']));
        }
        $product=$this->product(['stock'=>4]);Sanctum::actingAs($this->admin());
    ...
    {
        $manual=$this->product(['sku'=>'LUB-000002']);$this->assertSame(1,$manual->id);
        $generated=$this->product(['sku'=>null]);$this->assertSame(2,$generated->id);$this->assertSame('LUB-000002-1',$generated->sku);
    }

    public function test_backfill_reentry_does_not_overwrite_existing_operational_inventory(): void
    {
        $product=$this->product(['stock'=>4]);$inventory=WarehouseInventory::where('product_id',$product->id)->firstOrFail();
        app(InventoryService::class)->manualIn($product,$this->warehouse(),2,'Operación posterior');$beforeMovements=InventoryMovement::count();
        $migration=require database_path('migrations/2026_07_31_070150_backfill_initial_inventory_and_skus.php');$migration->up();
        $this->assertSame(6,$inventory->refresh()->quantity);$this->assertSame($beforeMovements,InventoryMovement::count());
    }
    ...
    public function test_partially_failed_backfill_rolls_back_its_prior_data_changes(): void
    {
        ...
        try{$migration->up();$this->fail('El backfill debía abortar.');}catch(\RuntimeException){}
    ...
    private function warehouse(): Warehouse{return Warehouse::where('code',Warehouse::INITIAL_CODE)->firstOrFail();}
    private function product(array $attributes=[]): Product
    {
        $stock=(int)($attributes['stock']??0);unset($attributes['stock']);
    ...
    private function branchPayload(string $code):array{return ['code'=>$code,'name'=>'Sede '.$code,'address'=>'Av. Prueba 123','is_active'=>true];}
    private function orderPayload(Product $product,int $quantity):array{return ['shipping_info'=>['address'=>'Av. Prueba 123','city'=>'Lima'],'payment_method'=>'transferencia','delivery_type'=>'delivery','items'=>[['product_id'=>$product->id,'quantity'=>$quantity]]];}
}
